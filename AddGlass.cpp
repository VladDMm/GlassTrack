...
void __fastcall TAddFormG::Initialize_Component()
{
    ...
    try {
        FDQuery1->SQL->Clear();
        FDQuery1->SQL->Add("SELECT DISTINCT a_marca FROM vehicle_table");
        FDQuery1->Open();
        ...
        FDQuery1->SQL->Clear();
        FDQuery1->SQL->Add("SELECT DISTINCT a_model FROM vehicle_table");
        FDQuery1->Open();
...
void __fastcall TAddFormG::CodeSearch()
{
    String cod_name;
...
void __fastcall TAddFormG::AddButtonClick(TObject* Sender)
{
	...
	// Verificam valorile introduse si le atribuim la string-urile specifice
    // care o sa fie adaugate in db
    marca = (AutoComboBox->Text != "") ? AutoComboBox->Text : AutoEdit->Text;
    model = (ModelComboBox->Text != "") ? ModelComboBox->Text : ModelEdit->Text;
    celula =
        (CelulaComboBox->Text != "") ? CelulaComboBox->Text : CelulaEdit->Text;
    cod_name = (CodComboBox->Text != "") ? CodComboBox->Text : PCodeEdit->Text;

    try {
        // Verificam daca automobilul exista
        FDQuery1->SQL->Text =
            "SELECT a_id FROM vehicle_table WHERE a_marca = :marca AND a_model = :model";
        ...
        if (!FDQuery1->IsEmpty()) {
			...
			// Adaugam automobilul
            FDQuery1->SQL->Text =
                "INSERT INTO vehicle_table (a_marca, a_model) VALUES (:marca, :model)";
            FDQuery1->ExecSQL();
            FDQuery1->SQL->Text = "SELECT LAST_INSERT_ID() AS new_a_id";
            FDQuery1->Open();
            a_id = FDQuery1->FieldByName("new_a_id")->AsInteger;// ultimul id pentru automobil ca sa facem legatura in tab product_auto_table
        }
        ...
        if (!FDQuery1->IsEmpty()) {
            ...
            FDQuery1->Open();
			celula_id = FDQuery1->FieldByName("celula_id")->AsInteger;// ultimul id pentru celula ca sa facem legatura in tab product_auto_table
        }
        ...
        if (!FDQuery1->IsEmpty()) {
            ...
            FDQuery1->Open();
			cod_id = FDQuery1->FieldByName("id_cod")->AsInteger;// ultimul id pentru cod ca sa facem legatura in tab product_auto_table
        }

		// Adaugam produs
        FDQuery1->SQL->Text =
            "INSERT INTO product_table (p_name, p_origine, p_count, p_price) VALUES (:p_name, :p_origine, :p_count, :p_price)";
		...
		FDQuery1->Open();
		p_id = FDQuery1->FieldByName("new_p_id")->AsInteger;// ultimul id pentru produs ca sa facem legatura in tab product_auto_table

		// Adaugam date in product_auto_table
        FDQuery1->SQL->Text =
            "INSERT INTO product_auto_table (a_id, p_id, celula_id, id_cod) VALUES (:a_id, :p_id, :celula_id, :id_cod)";
...
void __fastcall TAddFormG::AutoComboBoxChange(TObject* Sender)
{
    ...
    try {
        FDQuery1->Close();
        FDQuery1->SQL->Text =
            "SELECT a_model FROM vehicle_table WHERE a_marca = :marca";
...
// Functia care preia elementul selectat din comboboxul cu codul disponibil la apasarea
// tastei ENTER dacae, si pe baza lui se atribuie produsele si automobilul la codul dat,
    ...
    if (Key == VK_RETURN) {
        ...
        if (!FDQuery1->IsEmpty()) {
...

            {
                ...
                FDQuery1->SQL->Text = "SELECT a.a_marca, a.a_model "
                ...
                while (!FDQuery1->Eof) {
                    ...
                    if (ModelComboBox->Items->IndexOf(model) == -1)
                        ModelComboBox->Items->Add(model);
