    ...
    TForm(Owner), FDQuery1(query)
{
    ...
    logger->info(logger->charToWString(__func__).c_str(),
        L"Initializare constructor pentru TLoadDataForm.");
    LoadPrinters(); // Populează ComboBox-ul cu imprimante disponibile
    LoadSettings(); // Încarcă setările salvate
}

//---------------------------------------------------------------------------

// Funcție pentru a popula ComboBox-ul cu imprimantele disponibile
void __fastcall TLoadDataForm::LoadPrinters()
{
    PrinterComboBox->Items->Clear();

    for (int i = 0; i < Printer()->Printers->Count; i++) {
        PrinterComboBox->Items->Add(Printer()->Printers->Strings[i]);
    }

    if (PrinterComboBox->Items->Count > 0) {
        PrinterComboBox->ItemIndex = 0; // Selectează prima imprimantă
    }
}
...
void __fastcall TLoadDataForm::LoadSettings()
{
    ...
    if (FileExists(settingsFile)) {
        try {
            ...
            CopyCountEdit->Text = IntToStr(fileCopies);
...
// Ajustează dimensiunea coloanelor în StringGrid
...
// Functie care incarca un fisier CSV cu datele privitor la produse
void __fastcall TLoadDataForm::AddFileButtonClick(TObject* Sender)
{
    logger->info(logger->charToWString(__func__).c_str(),
        L"S-a apăsat butonul de adăugare fișier.");
    ...
    if (OpenDialog1->Execute()) // fereastra explorer
    {
        ...
        logger->info(logger->charToWString(__func__).c_str(),
            L"Fișier selectat: %s", filePath.c_str());

        TStringList* fileContent = new TStringList();
        try {
            fileContent->LoadFromFile(
                filePath, TEncoding::UTF8); // încarcă fișierul
            logger->debug(logger->charToWString(__func__).c_str(),
                L"Fișierul a fost încărcat cu succes. Număr linii: %d",
                fileContent->Count);

            if (fileContent->Count == 0) {
                logger->warning(WARN_NO_DATA_IN_FILE,
                    logger->charToWString(__func__).c_str(),
                    L"Fișierul este gol.");
                Application->MessageBox(
                    L"Fișierul este gol!", L"Eroare", MB_OK | MB_ICONERROR);
                delete fileContent;
                return;
            }
            ...
            logger->debug(logger->charToWString(__func__).c_str(),
                L"Prima linie are %d coloane.", firstLineColumns->Count);

            if (firstLineColumns->Count != 5) {
                logger->warning(WARN_NOT_FIVE_COLONS,
                    logger->charToWString(__func__).c_str(),
                    L"Fișierul nu are 5 coloane, ci %d.",
                    firstLineColumns->Count);
                Application->MessageBox(
                    L"Fișierul trebuie să aibă exact 5 coloane!", L"Eroare",
                    MB_OK | MB_ICONERROR);
            ...
            ResizeStringGrid();
            logger->info(logger->charToWString(__func__).c_str(),
                L"StringGrid resetat la 1 rând și 5 coloane.");
            ...
            for (int i = 0; i < fileContent->Count; i++) {
                ...
                logger->trace(logger->charToWString(__func__).c_str(),
                    L"Adăugată linia %d în StringGrid.", newRow);
            ...
            logger->warning(WARN_ERROR_READ_FILE,
                logger->charToWString(__func__).c_str(),
                L"Eroare la citirea fișierului: %s", filePath.c_str());
            Application->MessageBox(L"Eroare la citirea fișierului!", L"Eroare",
                MB_OK | MB_ICONERROR);
        }

        delete fileContent; // Eliberăm memoria pentru fișier
        logger->info(logger->charToWString(__func__).c_str(),
            L"Procesarea fișierului s-a terminat.");
    }
}
...
void __fastcall TLoadDataForm::ConfirmButtonClick(TObject* Sender)
{
    ...
    if (monthInterval.IsEmpty()) {
        monthInterval = "1"; // Valoare implicită de 1 lună
    } else {
        int intValue;
        if (!TryStrToInt(monthInterval, intValue))
    ...
    // Obține numele imprimantei selectate
    UnicodeString selectedPrinter = PrinterComboBox->Text;
    UnicodeString copyCount = CopyCountEdit->Text;
    if (copyCount == "")
        copyCount = "1";

    // Salvează imprimanta în settings.cfg
    try {
        UnicodeString settingsFile =
            ExtractFilePath(Application->ExeName) + L"settings.cfg";
        TStringList* fileContent = new TStringList();
        ...
        fileContent->Add(
            L"MonthInterval=" + monthInterval); // Adăugare interval de luni
    ...
    try {
        logger->info(logger->charToWString(__func__).c_str(),
            L"Pornire proces salvare date");
        for (int i = 1; i < StringGrid1->RowCount; i++) {
            logger->trace(logger->charToWString(__func__).c_str(),
                L"Procesare rând %d", i);
            ...
            logger->debug(logger->charToWString(__func__).c_str(),
                L"Verificare existență automobil %s", marca.c_str());
            newFDQuery->SQL->Text =
            ...
            if (!newFDQuery->IsEmpty()) {
                ...
                logger->debug(logger->charToWString(__func__).c_str(),
                    L"Automobil %s inexistent, adăugare în DB", marca.c_str());
                newFDQuery->SQL->Text =
            ...
            logger->debug(logger->charToWString(__func__).c_str(),
                L"Verificare existență celulă %s", celula.c_str());
            newFDQuery->SQL->Text =
            ...
            if (!newFDQuery->IsEmpty()) {
                ...
                logger->debug(logger->charToWString(__func__).c_str(),
                    L"Celulă %s inexistentă, adăugare în DB", celula.c_str());
                newFDQuery->SQL->Text =
            ...
            logger->debug(logger->charToWString(__func__).c_str(),
                L"Verificare existență cod %s", cod_name.c_str());
            newFDQuery->SQL->Text =
            ...
            if (!newFDQuery->IsEmpty()) {
                ...
                logger->debug(logger->charToWString(__func__).c_str(),
                    L"Cod %s inexistent, adăugare în DB", cod_name.c_str());
                newFDQuery->SQL->Text =
            ...
            if (newFDQuery->IsEmpty()) {
                logger->info(logger->charToWString(__func__).c_str(),
                    L"Adăugare produs nou în baza de date");
                newFDQuery->SQL->Text =
                    "INSERT IGNORE INTO product_auto_table (a_id, celula_id, id_cod, p_count, p_price, price_updated_at, is_updated) "
                    "VALUES (:a_id, :celula_id, :id_cod, :p_count, :p_price, CURRENT_TIMESTAMP, 'Actualizat')";
                newFDQuery->ParamByName("a_id")->AsInteger = a_id;
                newFDQuery->ParamByName("celula_id")->AsInteger = celula_id;
                newFDQuery->ParamByName("id_cod")->AsInteger = cod_id;
        ...
        logger->info(
            logger->charToWString(__func__).c_str(), L"Date salvate cu succes");
...
// eveniment redimensionare coloane din dbgrid
...
// Eveniment care inchide forma la ESC
void __fastcall TLoadDataForm::FormKeyPress(
    TObject* Sender, System::WideChar &Key)
{
    ...
    if (Key == VK_ESCAPE) {
        logger->info(logger->charToWString(__func__).c_str(),
            L"Fereastra este inchisa la apasarea tastei Escape");
        this->Close();
    }
}
//---------------------------------------------------------------------------

void __fastcall TLoadDataForm::ChangePassButtonClick(TObject* Sender)
{
    logger->info(
        logger->charToWString(__func__).c_str(), L"Functia a fost apelata");
    TTChangePassForm* ChangePassForm = new TTChangePassForm(this, FDQuery1);
    ChangePassForm->ShowModal();
    delete ChangePassForm;
    logger->info(
        logger->charToWString(__func__).c_str(), L"Functia a fost incheiata");
}
//---------------------------------------------------------------------------
