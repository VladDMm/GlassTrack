...
struct ProbeEnumerator {
  ...
  static auto getCurrentProp(T_ &t) -> decltype(t.Current)
  { return t.Current; }
...
private:
  ...
  typedef decltype(ProbeEnumerator<EnumeratorType>::getCurrentProp(enumerator)) ValueType;
...
private:
  ...
  TEnumerableIterator(SYSITERATOR_PTR T *ptr, EnumeratorType _enumerator)
    ...
    endReached = !ProbeEnumerator<EnumeratorType>::callMoveNext(enumerator);
  }

  TEnumerableIterator(SYSITERATOR_PTR T *ptr)
      : enumerable(ptr), offset(0), endReached(true), enumerator(EnumeratorType()) {}
...
public:
  ...
  TEnumerableIterator<T> &operator++() {
    if (ProbeEnumerator<EnumeratorType>::validate(enumerator) && !endReached) {
      endReached = !ProbeEnumerator<EnumeratorType>::callMoveNext(enumerator);
      offset++;
    }
    return *this;
  ...
  ValueType operator*() const { return ProbeEnumerator<EnumeratorType>::getCurrentProp(enumerator); }
  ValueType operator*()       { return ProbeEnumerator<EnumeratorType>::getCurrentProp(enumerator); }
...
auto CurrentMember(T &&t) -> decltype(t.Current)
{ return t.Current; }
...
auto back_inserter(SYSITERATOR_PTR T *container)
    -> TBackInsertIterator<T, decltype(CurrentMember(container->GetEnumerator()))> {
  return TBackInsertIterator<T, decltype(CurrentMember(container->GetEnumerator()))>(
      container);
}

template <typename T>
auto front_inserter(SYSITERATOR_PTR T *container)
  -> TInsertIterator<T, decltype(CurrentMember(container->GetEnumerator()))> {
  return TInsertIterator<T, decltype(CurrentMember(container->GetEnumerator()))>(
    container, 0);
}
