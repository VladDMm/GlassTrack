...
TAddFormG* AddFormG;
//---------------------------------------------------------------------------
__fastcall TAddFormG::TAddFormG(TComponent* Owner, logg::LogF *log) : TForm(Owner), logger(log) {}
//---------------------------------------------------------------------------
void __fastcall TAddFormG::CancelButtonClick(TObject* Sender)
{
    logger->info(logger->charToWString(__func__).c_str(), L"CancelButton apăsat. Închidere formă.");
    this->Close();
}
//---------------------------------------------------------------------------
void __fastcall TAddFormG::Initialize_Component()
{
	logger->info(logger->charToWString(__func__).c_str(), L"Initializare componentă AddForm.");
    FDQuery1->Connection = MenuForm->FDConnection1;

	try {
		logger->trace(logger->charToWString(__func__).c_str(), L"Executare SQL query: SELECT DISTINCT a_marca_model FROM vehicle_table");
        FDQuery1->SQL->Clear();
        ...
        AutoComboBox->Clear();
        while (!FDQuery1->Eof) {
            String value = FDQuery1->FieldByName("a_marca_model")->AsWideString;
			logger->debug(logger->charToWString(__func__).c_str(), L"Adăugare valoare la AutoComboBox: %s", value.c_str());
			AutoComboBox->Items->Add(value);
            FDQuery1->Next();
        }
        FDQuery1->Close();

        logger->trace(logger->charToWString(__func__).c_str(), L"Executare SQL query: SELECT DISTINCT nume_celula FROM celula_table");
        FDQuery1->SQL->Clear();
        ...
        while (!FDQuery1->Eof) {
            String value = FDQuery1->FieldByName("nume_celula")->AsString;
            logger->debug(logger->charToWString(__func__).c_str(), L"Adăugare valoare la CelulaComboBox: %s", value.c_str());
            CelulaComboBox->Items->Add(value);
            FDQuery1->Next();
        }
        FDQuery1->Close();

        logger->trace(logger->charToWString(__func__).c_str(), L"Executare SQL query: SELECT DISTINCT cod FROM code_table");
        FDQuery1->SQL->Clear();
        ...
        CodComboBox->Clear();
        while (!FDQuery1->Eof) {
            String value = FDQuery1->FieldByName("cod")->AsString;
			logger->debug(logger->charToWString(__func__).c_str(), L"Adăugare valoare la CodComboBox: %s", value.c_str());
			CodComboBox->Items->Add(value);
            FDQuery1->Next();
		}
		FDQuery1->Close();
	} catch (Exception &e) {
     std::wstring my_wstr = e.Message.c_str();
		std::string my_str =
            std::wstring_convert<std::codecvt_utf8_utf16<System::Char> > {}
				.to_bytes(my_wstr);
		logger->warning(WARN_DATA_LOAD_FAIL,logger->charToWString(__func__).c_str(), L"Eroare la încărcarea datelor: %s", my_str.c_str());
		ShowMessage(L"Eroare la încărcarea datelor: " + e.Message);
    }
}
...
void __fastcall TAddFormG::AddButtonClick(TObject* Sender)
{
	...
	{
        logger->warning(WARN_FIELDS_NOT_FILLED, logger->charToWString(__func__).c_str(), L"Câmpuri incomplete la adăugare");
		ShowMessage(L"Completează toate câmpurile!");
		return;
    }

	// Verificam valorile introduse
    logger->info(logger->charToWString(__func__).c_str(),L"Începere proces de adăugare");
	...
	try {
		logger->debug(logger->charToWString(__func__).c_str(), L"Verificare existență automobil");
        ...
        if (!FDQuery1->IsEmpty()) {
			...
			// Adaugam automobilul
			logger->info(logger->charToWString(__func__).c_str(), L"Adăugare nouă în vehicle_table");
			FDQuery1->SQL->Text =
		...
		logger->debug(logger->charToWString(__func__).c_str(), L"Verificare existență celulă");
        ...
        if (!FDQuery1->IsEmpty()) {
			...
			// Adaugam celula
			logger->info(logger->charToWString(__func__).c_str(), L"Adăugare nouă în celula_table");
            FDQuery1->SQL->Text =
        ...
        logger->debug(logger->charToWString(__func__).c_str(), L"Verificare existență cod");
        ...
        if (!FDQuery1->IsEmpty()) {
			...
			// Adaugam codul
			logger->info(logger->charToWString(__func__).c_str(), L"Adăugare nouă în code_table");
            FDQuery1->SQL->Text =
		...
		logger->debug(logger->charToWString(__func__).c_str(), L"Verificare existență în product_auto_table");
        FDQuery1->SQL->Text =
		...
		if (FDQuery1->IsEmpty()) {
			logger->info(logger->charToWString(__func__).c_str(), L"Inserare nouă în product_auto_table");
            ...
            logger->info(logger->charToWString(__func__).c_str(), L"Actualizare produs existent cu pa_id: %d", pa_Id);
            FDQuery1->SQL->Text =
		...
		logger->info(logger->charToWString(__func__).c_str(), L"Datele au fost salvate cu succes");
        ...
        std::string my_str =
            std::wstring_convert<std::codecvt_utf8_utf16<System::Char> > {}
				.to_bytes(my_wstr);
		logger->warning(WARN_SQL_INSERT, logger->charToWString(__func__).c_str(), L"Eroare la salvare: %s", my_wstr.c_str());
	...
	TObject* Sender, System::WideChar &Key)
{
	if (Key != VK_RETURN)
		return;
	logger->info(logger->charToWString(__func__).c_str(), L"Cod selecat la tasta ENTER");
	String cod_name = CodComboBox->Text;
	logger->debug(logger->charToWString(__func__).c_str(), L"Cod introdus: %s", cod_name.c_str());
	if (cod_name == "") {
		logger->debug(logger->charToWString(__func__).c_str(), L"Cod gol. Se încarcă listele auto și celule.");
		...
		logger->info(logger->charToWString(__func__).c_str(), L"Listele Auto și Celule încărcate.");
		return;
	}

	logger->info(logger->charToWString(__func__).c_str(), L"Cod selectat din combobox");
	...
	if (!FDQuery1->IsEmpty())
		cod_id = FDQuery1->FieldByName("id_cod")->AsInteger;
		logger->debug(logger->charToWString(__func__).c_str(), L"Cod găsit ID: %d", cod_id);
	} else {
		if (MessageDlg(
