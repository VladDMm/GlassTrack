...
// Constructor - înregistrează inițializarea formularului
__fastcall TLoadDataForm::TLoadDataForm(TComponent* Owner, TFDQuery* query, logg::LogF* log) :
	TForm(Owner), FDQuery1(query)
{
	this->logger = log;
	this->logger->info(logger->charToWString(__func__).c_str(), L"Initializare constructor pentru TLoadDataForm.");
}

//---------------------------------------------------------------------------

// Ajustează dimensiunea coloanelor în StringGrid și loghează informații
void __fastcall TLoadDataForm::ResizeStringGrid()
{
    int colCount = StringGrid1->ColCount;
    int totalWidth = StringGrid1->Width - 20; // Lăsăm puțin spațiu pentru scrollbar
...
{
	logger->info(logger->charToWString(__func__).c_str(), L"S-a apăsat butonul de adăugare fișier.");
	...
	if (OpenDialog1->Execute()) // fereastra explorer
	{
		...
		logger->info(logger->charToWString(__func__).c_str(), L"Fișier selectat: %s", filePath.c_str());

		TStringList* fileContent = new TStringList();
		try {
			fileContent->LoadFromFile(filePath, TEncoding::UTF8); // încarcă fișierul
			logger->debug(logger->charToWString(__func__).c_str(), L"Fișierul a fost încărcat cu succes. Număr linii: %d", fileContent->Count);

			if (fileContent->Count == 0) {
				logger->warning(WARN_NO_DATA_IN_FILE, logger->charToWString(__func__).c_str(), L"Fișierul este gol.");
			...
			logger->debug(logger->charToWString(__func__).c_str(), L"Prima linie are %d coloane.", firstLineColumns->Count);

			if (firstLineColumns->Count != 5) {
				logger->warning(WARN_NOT_FIVE_COLONS, logger->charToWString(__func__).c_str(), L"Fișierul nu are 5 coloane, ci %d.", firstLineColumns->Count);
			...
			ResizeStringGrid();
			logger->info(logger->charToWString(__func__).c_str(), L"StringGrid resetat la 1 rând și 5 coloane.");
			...
			for (int i = 0; i < fileContent->Count; i++) {
				...
				logger->trace(logger->charToWString(__func__).c_str(), L"Adăugată linia %d în StringGrid.", newRow);

				delete columns;
			}
		} catch (...) {
			logger->warning(WARN_ERROR_READ_FILE, logger->charToWString(__func__).c_str(), L"Eroare la citirea fișierului: %s", filePath.c_str());
		...
		logger->info(logger->charToWString(__func__).c_str(), L"Procesarea fișierului s-a terminat.");
	}
}
...
void __fastcall TLoadDataForm::ConfirmButtonClick(TObject* Sender)
{
	...
	try {
		logger->info(logger->charToWString(__func__).c_str(), L"Pornire proces salvare date");
		for (int i = 1; i < StringGrid1->RowCount; i++) {
		   logger->trace(logger->charToWString(__func__).c_str(), L"Procesare rând %d", i);
			...
			int p_count = StrToFloatDef(fixNumberFormat(StringGrid1->Cells[3][i]), 0.0);
			double p_price = StrToFloatDef(fixNumberFormat(StringGrid1->Cells[4][i]), 0.0);

		   logger->debug(logger->charToWString(__func__).c_str(), L"Verificare existență automobil %s", marca.c_str());
			newFDQuery->SQL->Text = "SELECT a_id FROM vehicle_table WHERE a_marca_model = :marca";
			...
			if (!newFDQuery->IsEmpty()) {
				...
				logger->debug(logger->charToWString(__func__).c_str(), L"Automobil %s inexistent, adăugare în DB", marca.c_str());
				newFDQuery->SQL->Text = "INSERT INTO vehicle_table (a_marca_model) VALUES (:marca)";
				newFDQuery->ExecSQL();
			...
			logger->debug(logger->charToWString(__func__).c_str(), L"Verificare existență celulă %s", celula.c_str());
			newFDQuery->SQL->Text = "SELECT id_celula FROM celula_table WHERE nume_celula = :nume_celula";
			...
			if (!newFDQuery->IsEmpty()) {
				...
				logger->debug(logger->charToWString(__func__).c_str(), L"Celulă %s inexistentă, adăugare în DB", celula.c_str());
				newFDQuery->SQL->Text = "INSERT INTO celula_table (nume_celula) VALUES (:nume_celula)";
				newFDQuery->ExecSQL();
			...
			logger->debug(logger->charToWString(__func__).c_str(), L"Verificare existență cod %s", cod_name.c_str());
			newFDQuery->SQL->Text = "SELECT id_cod FROM code_table WHERE cod = :cod_name";
			...
			if (!newFDQuery->IsEmpty()) {
				...
				logger->debug(logger->charToWString(__func__).c_str(), L"Cod %s inexistent, adăugare în DB", cod_name.c_str());
				newFDQuery->SQL->Text = "INSERT INTO code_table (cod) VALUES (:cod_name)";
				newFDQuery->ExecSQL();
			...
			newFDQuery->SQL->Text = "SELECT 1 FROM product_auto_table WHERE a_id = :a_id AND celula_id = :celula_id AND id_cod = :id_cod";
			...
			if (newFDQuery->IsEmpty()) {
				logger->info(logger->charToWString(__func__).c_str(), L"Adăugare produs nou în baza de date");
				newFDQuery->SQL->Text = "INSERT IGNORE INTO product_auto_table (a_id, celula_id, id_cod, p_count, p_price) VALUES (:a_id, :celula_id, :id_cod, :p_count, :p_price)";
                newFDQuery->ParamByName("a_id")->AsInteger = a_id;
                newFDQuery->ParamByName("celula_id")->AsInteger = celula_id;
                newFDQuery->ParamByName("id_cod")->AsInteger = cod_id;
		...
		logger->info(logger->charToWString(__func__).c_str(), L"Date salvate cu succes");
        ShowMessage(L"Datele au fost salvate cu succes!");
        delete newFDQuery;
        ModalResult = mrOk;
	} catch (Exception &e) {
		logger->warning(WARN_DATA_PUSH_DATA_DB, logger->charToWString(__func__).c_str(), L"Eroare la salvare: %s", e.Message.c_str());
...
{
	logger->trace(logger->charToWString(__func__).c_str(), L"Tasta apasata: %c", Key);
	if(Key == VK_ESCAPE) {
		logger->info(logger->charToWString(__func__).c_str(), L"Fereastra este inchisa la apasarea tastei Escape");
        this->Close();
    }
}
//---------------------------------------------------------------------------
