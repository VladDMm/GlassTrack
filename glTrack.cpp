...
bool __fastcall TMenuForm::VerifyOldPassword(const UnicodeString &oldPassword)
{
    ...
    try {
        ...
        delete newFdQuery;
        logger->warning(UNDEFINED_WARN, logger->charToWString(__func__).c_str(),
            L"Eroare necunoscuta");
        return false;
    }
}
//-------------------------------------------------------------------------
    ...
    const String &filePath, String &printerName, int &numCopies)
{
    ...
    delete fileContent;
    logger->info(logger->charToWString(__func__).c_str(), L"Functie incheiata");
}
    ...
    const String &celula, double pret, int cantitate)
{
    ...
    if (Prntr->Printing) {
        ...
        Prntr->EndDoc();
        logger->info(
            logger->charToWString(__func__).c_str(), L"Functie incheiata");
    }
}
        ...
        if (FileExists(settingsFile)) {
            ...
            for (int i = 0; i < fileContent->Count; i++) {
                ...
                if (line.Pos(L"MonthInterval=") == 1) {
                    monthInterval =
                        StrToIntDef(line.SubString(15, line.Length() - 14), 1);
                    break;
                }
            }
        }
        ...
        case VK_DELETE: // Stergere Produs
        {
            ...
            try {
                ...
                logger->warning(WARN_DELETE_FAILED,
                    logger->charToWString(__func__).c_str(),
                    L"Eroare la ștergere: %s", str.w_str());
