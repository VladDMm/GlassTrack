...
void LogF::ensureLogFileExists()
{
    std::filesystem::path logPath =
        std::filesystem::current_path() / baseFilePathToLog;
    ...
    if (!std::filesystem::exists(logPath.parent_path())) {
        if (!std::filesystem::create_directories(logPath.parent_path(), ec)) {
            std::cerr << "Error: Unable to create directory: "
                      << logPath.parent_path() << " - " << ec.message()
                      << std::endl;
            return;
        }
    }
    ...
    if (!std::filesystem::exists(logPath)) {
        ...
        if (!createFile) {
            std::cerr << "Error: Unable to create log file: " << logPath
                      << std::endl;
            return;
        }
...
std::string LogF::addSpaces(std::string str, size_t len)
{
    ...
    while (strLocal.length() <
           len) // Daca lungimea sirului este mai mica decat lungimea necesara
...
void LogF::setFilePath(const char* file_path, const char* file_name)
{
    ...
    if (!fpLog) {
        std::cerr << "Failed to open log file: " << baseFilePathToLog
                  << std::endl;
    }
}
...
void LogF::setLevelLog(unsigned short logLevel)
{
    ...
    switch (logLevel) {
        case 1:
            _isLevelException = true;
            msgLevel = EXCEPTION;
            break; // nivel exception
        case 2:
            _isLevelException = true;
            _isLevelWarning = true;
            msgLevel = WARNING;
            break; // nivel exception + warning
        case 3:
            _isLevelException = true;
            _isLevelInfo = true;
            _isLevelWarning = true;
            msgLevel = INFO;
            break; // nivel exception + warning + info
        case 4:
            _isLevelException = true;
            _isLevelDebug = true;
            _isLevelWarning = true;
            _isLevelInfo = true;
            msgLevel = DEBUG;
            break; // nivel exception + warning + info + debug
        case 5:
            _isLevelException = true;
            _isLevelDebug = true;
            _isLevelWarning = true;
            ...
            _isLevelTrace = true;
            msgLevel = TRACE;
            break; // nivel exception + warning + info + debug + trace
        default:
            _isLevelNone = true;
            msgLevel = NONE;
            if (fpLog.is_open()) {
    ...
    if (!fpLog.is_open()) {
        fpLog.open(baseFilePathToLog, std::fstream::out | std::fstream::app);
        if (!fpLog) {
            std::cerr << "Error: Could not open log file: " << baseFilePathToLog
                      << std::endl;
        }
    }
}
...
void LogF::info(const wchar_t* functionName, const wchar_t* format, ...)
{
    if (!_isLevelInfo || _isLevelNone || !fpLog.is_open())
        return;
...
buffer[_countof(buffer) - 1] = L'\0';
    ...
    // Conversie `wchar_t*` → `std::string` (UTF-8)
    std::wstring_convert<std::codecvt_utf8<wchar_t> > converter;
    std::string functionNameStr = converter.to_bytes(functionName);
    std::string bufferStr = converter.to_bytes(buffer);

    // construire mesajul de log
    std::string formatMessage =
        addSpaces(getCurrentTime(), 21) + addSpaces(GetPID(), 13) +
        addSpaces("Info", 21) + addSpaces(functionNameStr, 23) +
        addSpaces("", 15) + bufferStr + "\n";

    fpLog << formatMessage;
    fpLog.flush();
}

void LogF::debug(const wchar_t* functionName, const wchar_t* format, ...)
{
    if (!_isLevelDebug || _isLevelNone || !fpLog.is_open())
        return;
...
buffer[_countof(buffer) - 1] = L'\0';
    ...
    // Construire mesaj de log
    std::string formatMessage =
        addSpaces(getCurrentTime(), 21) + addSpaces(GetPID(), 13) +
        addSpaces("Debug", 21) + addSpaces(functionNameStr, 23) +
        addSpaces("", 15) + bufferStr + "\n";
...
{
    if (!_isLevelException || _isLevelNone || !fpLog.is_open())
        return;
...
buffer[_countof(buffer) - 1] = L'\0';
    ...
    std::string formatMessage =
        addSpaces(getCurrentTime(), 21) + addSpaces(GetPID(), 13) +
        addSpaces("Exception", 21) + addSpaces(functionNameStr, 23) +
        addSpaces("L" + std::to_string(lineNumber), 15) + bufferStr + "\n";
...
{
    if (!_isLevelWarning || _isLevelNone || !fpLog.is_open())
        return 0;
...
buffer[_countof(buffer) - 1] = L'\0';
    ...
    std::string formatMessage =
        addSpaces(getCurrentTime(), 21) + addSpaces(GetPID(), 13) +
        addSpaces("Warning", 21) + addSpaces(functionNameStr, 23) +
        addSpaces(std::to_string(warningCode), 15) + bufferStr + "\n";
...
void LogF::trace(const wchar_t* functionName, const wchar_t* format, ...)
{
    if (!_isLevelTrace || _isLevelNone || !fpLog.is_open())
        return;
...
buffer[_countof(buffer) - 1] = L'\0';
    ...
    // Conversie din `wchar_t*` în `std::string`
    std::wstring_convert<std::codecvt_utf8<wchar_t> > converter;
    std::string functionNameStr = converter.to_bytes(functionName);
    std::string bufferStr = converter.to_bytes(buffer);
    ...
    std::string formatMessage =
        addSpaces(getCurrentTime(), 21) + addSpaces(GetPID(), 13) +
        addSpaces("Trace", 21) + addSpaces(functionNameStr, 23) +
        addSpaces("", 15) + bufferStr + "\n";
...
void LogF::checkAndWriteHeader()
{
    ...
    std::string currentDate =
        getCurrentTime().substr(0, 10); // Extrage doar "YYYY-MM-DD"
    if (lastDate.size() >= 9) {
        lastDate.erase(lastDate.begin()+10);
	}
...
std::wstring LogF::charToWString(const char* charStr)
{
    size_t len = std::mbstowcs(nullptr, charStr, 0) + 1;
    std::wstring wstr(len, L'\0');
    std::mbstowcs(&wstr[0], charStr, len);
    return wstr;
}

void LogF::add_end_line_of_program()
{
	fpLog << "\n|============================================="
	<<"=================================================================================================|\n";
	fpLog << getCurrentTime() << "\n\n";
    fpLog.flush();
}
